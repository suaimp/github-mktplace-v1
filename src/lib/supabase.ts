...
export async function resetPassword(email: string) {
  try {
    ...
    if (!checkRateLimit(rateLimitKey)) {
      const remainingMinutes = getRateLimitReset(rateLimitKey);
      throw new Error(`Muitas tentativas. Por favor, aguarde ${remainingMinutes} minutos antes de tentar novamente.`);
    }
...
export async function getRedirectPath(): Promise<string> {
  try {
    ...
    if (user.type === 'platform') {
      ...
      return role === 'publisher' ? '/publisher/dashboard' : '/advertiser/dashboard';
    }
