...
export default function MarketplaceTable({ formId }: MarketplaceTableProps) {
  ...
  // Calculate tab counts using the cached data - MEMOIZED to avoid calling on every render
  const tabCounts = useMemo(() => calculateTabCounts(), [calculateTabCounts]);
  
  // Definir tabs para filtros do marketplace com contadores
  const tabs = [
    { id: 'todos', label: 'Todos os Sites', count: tabCounts.todos },
    { id: 'promocao', label: 'Promoção', count: tabCounts.promocao },
    { id: 'favoritos', label: 'Favoritos', count: tabCounts.favoritos }
  ];
  ...
  // Create filtered entries from hook data
  const filteredEntries = entries; // The hook already handles filtering
  const ordersPerPage = itemsPerPage; // Map itemsPerPage to ordersPerPage for compatibility
  
  // Create sortState object for compatibility
  const sortState = {
    field: sortField,
    ...
    <div className="w-full relative overflow-hidden">
      {selectedEntries.length > 0 && (
        <BulkSelectionBar
          ...
          entries={entries}
          fields={fields}
          formTitle="Marketplace - Sites Selecionados"
        />
      )}
