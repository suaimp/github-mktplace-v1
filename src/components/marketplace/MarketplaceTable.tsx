...
export default function MarketplaceTable({ formId }: MarketplaceTableProps) {
  ...
  async function loadMarketplaceData() {
    try {
      ...
      const sortFieldData = fieldsData.find(field => 
        field.form_field_settings?.sort_by_field === true
      );
  ...
  const productNameField = fields.find(field => field.form_field_settings?.is_product_name === true);
  const productUrlField = fields.find(field => field.form_field_settings?.is_site_url === true);
  const productPriceField = fields.find(field => field.field_type === 'product');

  // Find button buy field that should be positioned in last column
  const buttonBuyField = fields.find(field => 
  ...
  const renderApiMetricWithBadge = (value: any, fieldType: string) => {
    ...
    return (
      <div className="flex items-center">
        <span>{value}</span>
        {!isNaN(numValue) && ['moz_da', 'semrush_as', 'ahrefs_dr'].includes(fieldType) && (
        ...
        <div className="max-w-full overflow-x-auto">
          <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-800">
            <thead className="bg-gray-50 dark:bg-gray-800">
              <tr>
                <th scope="col" className="w-10 px-3 py-3.5 text-left text-sm font-semibold text-gray-900 dark:text-white">
                  <div className="flex items-center">
                    <div className="relative">
                      <input
                        type="checkbox"
                ...
                {tableFields.map(field => {
                  ...
                  const isSortable = field.field_type === 'number' || 
                                    ['moz_da', 'semrush_as', 'ahrefs_dr', 
                                     'ahrefs_traffic', 'similarweb_traffic', 
                                     'google_traffic'].includes(field.field_type);
                ...
                {buttonBuyField && (
                  <th scope="col" className="px-3 py-3.5 text-left text-sm font-semibold text-gray-900 dark:text-white">
                    {buttonBuyField.form_field_settings?.marketplace_label || buttonBuyField.label}
                  </th>
                )}
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-200 bg-white dark:divide-gray-800 dark:bg-gray-900">
              {paginatedEntries.map(entry => {
                ...
                return (
                  <tr key={entry.id} className={selectedEntries.includes(entry.id) ? 'bg-brand-50 dark:bg-brand-900/10' : ''}>
                    ...
                    {tableFields.map(field => {
                      ...
                      if (['moz_da', 'semrush_as', 'ahrefs_dr'].includes(field.field_type)) {
                        return (
                          <td key={field.id} className="whitespace-nowrap px-3 py-4 text-sm text-gray-700 dark:text-gray-300">
                            {renderApiMetricWithBadge(entry.values[field.id], field.field_type)}
                          </td>
                        );
                      }
                    ...
                    {buttonBuyField && (
                      <td className="whitespace-nowrap px-3 py-4 text-sm">
                        <AddToCartButton
                          ...
                          price={productPrice}
                          url={productUrl}
                          buttonStyle={buttonBuyField.form_field_settings?.button_style || 'primary'}
                          buttonText={buttonBuyField.form_field_settings?.custom_button_text ? buttonBuyField.form_field_settings?.button_text : buttonBuyField.label}
                          isInCart={isInCart}
                        />
                      </td>
                    )}
