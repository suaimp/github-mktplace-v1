...
export default function ApiMetricBadge({ value, fieldType }: ApiMetricBadgeProps) {
  // Get score letter and colors based on value
  const getScoreInfo = (value: number): { letter: string, bgColor: string, textColor: string } => {
    if (value >= 80) {
      return {
        letter: 'A',
        bgColor: '#9EF2C9',
  ...
  const shouldShowScore = ['moz_da', 'semrush_as', 'ahrefs_dr'].includes(fieldType);
