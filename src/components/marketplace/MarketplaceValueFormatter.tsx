...
export function renderCountryFlags(countries: Record<string, any>, showCountryCodes: boolean = false) {
...
export function formatMarketplaceValue(value: any, fieldType: string, showCountryCodes: boolean = false) {
        ...
        return (
          <div className="flex flex-wrap gap-1">
            {value.map((item, index) => (
              <span key={index} className="inline-flex rounded-full bg-brand-50 px-2 py-0.5 text-theme-xs font-medium text-brand-500 dark:bg-brand-500/15 dark:text-brand-400">
                {item}
              </span>
            ))}
          </div>
