...
export default function CountrySettings({ settings, onChange }: CountrySettingsProps) {
              ...
              <div key={country.code} className="flex items-center gap-3">
                <input
                  ...
                  onChange={(e) => {
                    const newCountries = e.target.checked
                      ...
                      : (settings.countries || []).filter(c => c !== country.code);
                ...
                <div className="flex items-center gap-2">
                  <img 
                    ...
                    height="24"
                    alt={country.name}
                    className={`rounded-full ${country.code === 'ROW' ? 'dark:invert' : ''}`}
                  />
          ...
          {multiSelect && (
            <div>
              ...
              <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
                Limit how many countries users can select. Leave empty to allow unlimited selections.
              </p>
            </div>
          )}
        </div>
        ...
        <div>
          <div className="flex items-center gap-2">
            <Switch
              ...
              onChange={(checked) => onChange({ ...settings, show_percentage: checked })}
            />
          </div>
          <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
            Display country codes (BR, US, etc.) alongside flags in tables and forms
          </p>
        </div>
      </div>
    </div>
