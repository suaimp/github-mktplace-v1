...
serve(async (req)=>{
  ...
  const { data: settingsArr, error: settingsError } = await supabase.from("pagarme_settings").select("pagarme_test_mode").order("created_at", {
    ascending: true
  }).limit(1);
  ...
  try {
    ...
    if (body.action === 'tokenize') {
      ...
      if (!card_number || !card_exp_month || !card_exp_year || !card_cvv || !card_holder_name) {
        return new Response(JSON.stringify({
          error: 'Dados do cartão incompletos para tokenização'
        }), {
          status: 400,
          headers: corsHeaders
        });
      ...
      try {
        ...
        return new Response(JSON.stringify({
          error: 'Erro inesperado na tokenização',
          details: String(err)
        }), {
          status: 500,
          headers: corsHeaders
    ...
    if (body.action === 'payment_with_token' || !body.action) {
      ...
      if (!body.items || !body.customer || !body.payments) {
        return new Response(JSON.stringify({
          error: 'Payload de pagamento incompleto.'
        }), {
          status: 400,
          headers: corsHeaders
        });
