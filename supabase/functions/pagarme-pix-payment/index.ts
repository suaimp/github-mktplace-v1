...
serve(async (req) => {
  ...
  if (!body) {
    return new Response(JSON.stringify({ error: 'Body não enviado' }), { status: 400, headers: corsHeaders });
  }
  ...
  try {
    ...
    if (!amount || !customer_name || !customer_email || !customer_document) {
      return new Response(JSON.stringify({ 
        ...
        required: ['amount', 'customer_name', 'customer_email', 'customer_document'],
        received: { amount, customer_name, customer_email, customer_document }
      }), {
        status: 400,
        headers: corsHeaders,
      });
    ...
    if (!customerValidation.name || !customerValidation.email || !customerValidation.document || 
        !customerValidation.document_type || !pixValidation.expires_in) {
      return new Response(JSON.stringify({ 
    ...
    try {
      ...
      if (pagarmeData.errors && pagarmeData.errors.length > 0) {
        ...
        const errorMessages = pagarmeData.errors.map((err: any) => err.message || err.type || 'Erro desconhecido').join(', ');
        return new Response(JSON.stringify({ 
      ...
      if (!finalHasQrCode || !finalHasQrCodeUrl) {
        ...
        const diagnostics = {
          ...
          pix_keys_present: pagarmeData.charges?.[0]?.last_transaction?.pix ? 
            Object.keys(pagarmeData.charges[0].last_transaction.pix) : [],
