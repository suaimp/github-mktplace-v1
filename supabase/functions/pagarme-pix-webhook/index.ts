...
serve(async (req) => {
  ...
  if (body && body.type && body.data) {
    ...
    if (eventType === "charge.paid" && cleanChargeId) {
      ...
      const { data: order, error } = await supabase
        ...
        .eq("payment_id", cleanChargeId)
        .maybeSingle();
      console.log("[WEBHOOK] Resultado da busca pelo pedido:", { order, error });
      if (!order) {
        console.error("[WEBHOOK] Pedido não encontrado para payment_id:", cleanChargeId);
        return new Response(JSON.stringify({ success: false, error: "Pedido não encontrado" }), { status: 200, headers: corsHeaders });
      }
      await supabase
        .from("orders")
  ...
  return new Response(JSON.stringify({ error: 'Webhook inválido' }), { status: 400, headers: corsHeaders });
}); 